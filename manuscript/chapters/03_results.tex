...
\subsection{Interactome Topology and WGCNA Module--Trait Relationships}

The reconstructed Protein--Protein Interaction network (28 nodes, 33 high-confidence edges; Fig.~\ref{fig:network}a) identified four interconnected topological clusters:
\begin{enumerate}
    \item \textbf{Vascular Secondary Wall Hub:} Anchored by CESA4, CESA7, IRX9, and IRX10 (betweenness centrality = 0.42);
    \item \textbf{Actin-Myosin Secretory Cable:} Comprising MYA1, MYA2, XI-K, VLN1, and ARP2/3 complexes;
    \item \textbf{Cortical Microtubule--CSC Guide Complex:} Mediated by CSI1/POM2 bridging cortical MTs (MAP65, SPR1, CLASP) to primary CESAs;
    \item \textbf{Intracellular Glycosylation Regulatory Axis:} SEC and SPY directly linking to motor subunits and MT plus-end trackers.
\end{enumerate}

\begin{figure*}[t]
\centering
\includegraphics[width=0.48\textwidth]{figures/05_cytoskeleton_glycome_interactome.png}
\includegraphics[width=0.48\textwidth]{figures/06_wgcna_module_trait_relationships.png}
\caption{\textbf{Systems Interactome and WGCNA Module--Trait Correlations.} \textbf{a}, Cytoskeleton--glycome functional PPI network. \textbf{b}, WGCNA Module--Trait correlation matrix between co-expression eigengenes and OSD-615 glycan class vectors.}
\label{fig:network}
\end{figure*}

WGCNA co-expression modeling resolved four gene modules (Fig.~\ref{fig:network}b). The \textit{Turquoise Module} (Vascular Secondary Wall / Xylan) exhibited a strong positive correlation with xylan epitope abundance ($r = +0.86$, $p = 0.0003$). The \textit{Blue Module} (Actin-Myosin Secretory Streaming) correlated positively with non-fucosylated xyloglucans ($r = +0.78$, $p = 0.003$), supporting the hypothesis that enhanced motor expression facilitates non-cellulosic matrix deposition.
